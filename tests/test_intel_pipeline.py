...
既定オフライン（SampleFactCollector＋DeterministicReporter）。index.html には触れない。
"""
import json
...
def test_run_intel_is_deterministic(tmp_path):
    ...
    run_intel(NOW, tmp_path / "a")
    run_intel(NOW, tmp_path / "b")
